...
public class MainActivity extends Activity {
	...
	private final boolean DEBUG_ENABLE = true; //音视频是否保存在本地

	//rtmp://192.168.1.102:1935/live/stream  推流路径
	private String _rtmpUrl = "rtmp://192.168.8.59:1935/live/12345";
	...
	private RtmpSessionManager _rtmpSessionMgr = null;////会话管理器
	...
	private Runnable _aacEncoderRunnable = new Runnable() {
		...
		public void run() {
			...
			if (DEBUG_ENABLE) {
				...
				Log.i(LOG_TAG,"======strFilename====aac====="+strFilename);
				try {
	...
	private int getDisplayOritation(int degrees, int cameraId) {  //设置显示方向
	...
	private void Start() {
		if (DEBUG_ENABLE) {
			...
			Log.i(LOG_TAG,"==============strFilename=========="+strFilename);
			try {
				if (!new File(strFilename).exists()) {
					new File(strFilename).createNewFile();
				}
	...
	private void InitAudioRecord() {
		...
		_fdkaacEnc = new FdkAacEncode();  //实例化音频编码器
